...
class Character extends GravitySprite {
    ...
    private scale_: number = 0.2;
    private scene_: Scene;
    public frozen_: boolean = false;

    /* animation related */
    private lock_ = false;
    private animating_: boolean = false;
    private animationProgress_: number = 0;
    private animationDirection_: number = 1;
    private interval_: number;
    private originalXPos_: number;
    private originalYPos_: number;

    private rolling_: boolean = false;

    // used for smooth transitions
    private initPos_: number[];
    private noTurn_ = false;

    /* jump related */
    private charged_: boolean = false;
    private jumping_: boolean = false;
    private jumpProgress_: number = 0;
    private direction_: string = "any";

    /* body parts */
    private head_: BodyGroup;
    private body_: BodyPart;
    private body_overlay_: BodyPart;
    private tail_: BodyPart;
    private left_arm_: BodyGroup;
    private right_arm_: BodyGroup;
    private left_leg_: BodyGroup;
    private right_leg_: BodyGroup;

    /* mouse caption */
    private mouse_: any;

    /* equipment */
    private hasKnife_: boolean = true;
    private belt_knife_: BodyPart;
    private holstering_: boolean = true;
    private rifle_: Sprite;
    private rifle_holstered_: Sprite;
    private ammo_tape_: Sprite;
    ...
    public constructor(scene: Scene) {
        ...
        /* body part setup */
        // head
        this.head_ = new BodyGroup("head", 
            new BodyPart("head_1", 260.062 * this.scale_, 330.929 * this.scale_), 
            new BodyPart("eye_3", 260.062 * this.scale_, 330.929 * this.scale_)
        );
        // main body
        ...
        // arms
        this.left_arm_ = new BodyGroup(
            "left_arm",
            new BodyPart("top_arm_part", 145.572 * this.scale_, 83.115 * this.scale_),
            new BodyGroup(
                "bottom_arm_w_knife",
                new BodyPart("bottom_arm_part", 288.596 * this.scale_, 96.311 * this.scale_),
                new BodyPart("knife_blade", 185.025 * this.scale_, 53.5 * this.scale_)
            )
        );
        this.right_arm_ = new BodyGroup("right_arm",
            new BodyPart("top_arm_part", 145.572 * this.scale_, 83.115 * this.scale_),
            new BodyPart("bottom_arm_part_bis", 288.596 * this.scale_, 96.311 * this.scale_)
        );
        // legs
        this.left_leg_ = new BodyGroup(
            "left_leg",
            new BodyPart("top_leg_part", 143.5 * this.scale_, 87.521 * this.scale_),
            new BodyGroup("left_foot", 
                new BodyPart("bottom_leg_part", 166.552 * this.scale_, 82.375 * this.scale_),
                new BodyPart("foot", 48.669 * this.scale_, 147.651 * this.scale_)
            )
        );
        this.right_leg_ = new BodyGroup("right_leg",
            new BodyPart("top_leg_part", 143.5 * this.scale_, 87.521 * this.scale_),
            new BodyGroup("right_foot", 
                new BodyPart("bottom_leg_part", 166.552 * this.scale_, 82.375 * this.scale_),
                new BodyPart("foot", 48.669 * this.scale_, 147.651 * this.scale_)
            )
        );
    ...
    public spawn(x: number, y: number) {
        ...
        this.left_arm_.getPart(1).setPivotRotation(0.35 * (this.left_arm_.getPart(1) as BodyGroup).getPart(0).getLargeur(), 0.17 * (this.left_arm_.getPart(1) as BodyGroup).getPart(0).getHauteur());
        ...
        (this.left_leg_.getPart(1) as BodyGroup).getPart(0).setXY(0, 0);
        (this.left_leg_.getPart(1) as BodyGroup).getPart(1).setXY(-68 * this.scale_, -24 * this.scale_ + (this.left_leg_.getPart(1) as BodyGroup).getPart(0).getHauteur());
        ...
        this.left_leg_.getPart(1).setPivotRotation(0.37 * (this.left_leg_.getPart(1) as BodyGroup).getPart(0).getLargeur(), 0.1 * (this.left_leg_.getPart(1) as BodyGroup).getPart(0).getHauteur());
        (this.left_leg_.getPart(1) as BodyGroup).getPart(1).setPivotRotation(0.8 * (this.left_leg_.getPart(1) as BodyGroup).getPart(1).getLargeur(), 0.55 * (this.left_leg_.getPart(1) as BodyGroup).getPart(1).getHauteur());
        ...
        (this.right_leg_.getPart(1) as BodyGroup).getPart(0).setXY(0, 0);
        (this.right_leg_.getPart(1) as BodyGroup).getPart(1).setXY(-68 * this.scale_, -24 * this.scale_ + (this.right_leg_.getPart(1) as BodyGroup).getPart(0).getHauteur());
        ...
        this.right_leg_.getPart(1).setPivotRotation(0.37 * (this.right_leg_.getPart(1) as BodyGroup).getPart(0).getLargeur(), 0.1 * (this.right_leg_.getPart(1) as BodyGroup).getPart(0).getHauteur());
        (this.right_leg_.getPart(1) as BodyGroup).getPart(1).setPivotRotation(0.8 * (this.right_leg_.getPart(1) as BodyGroup).getPart(1).getLargeur(), 0.55 * (this.right_leg_.getPart(1) as BodyGroup).getPart(1).getHauteur());
        ...
        /* knife */
        (this.left_arm_.getPart(1) as BodyGroup).getPart(1).setRotation(75);
        (this.left_arm_.getPart(1) as BodyGroup).getPart(1).getBalise().style.transform += " scaleX(-1)";
        (this.left_arm_.getPart(1) as BodyGroup).getPart(1).setXY(-24 * this.scale_, -140 * this.scale_ + (this.left_arm_.getPart(1) as BodyGroup).getPart(0).getHauteur());
        (this.left_arm_.getPart(1) as BodyGroup).getPart(1).masquer();
        ...
        let int: number = window.setInterval(() => {
    ...
    public moveTo(x: number, y: number) { // update setXY() method
        this.setXY(x, y);
        this.charFall(this.targetFall());
    }
    ...
    */
    ...
    private animate(animation: Function, delay: number) {
        /* loops an animation */
        if (this.animating_) {
            this.stop();
        }
    ...
    private idle() {
        if (this.animating_) {
            ...
            (this.left_leg_.getPart(1) as BodyGroup).getPart(1).setRotation(0 + this.animationProgress_ * 10);
            ...
            (this.right_leg_.getPart(1) as BodyGroup).getPart(1).setRotation(0 + this.animationProgress_ * 10);
    ...
    private walk(speed: number) {
        ...
        (this.right_leg_.getPart(1) as BodyGroup).getPart(1).setRotation(20 - 20 * this.animationProgress_);
        ...
        (this.left_leg_.getPart(1) as BodyGroup).getPart(1).setRotation(0 + 20 * this.animationProgress_);

        this.animationProgress_ += this.animationDirection_ * speed;
    }
    private jump(hight: number, speed: number) { // needs to be blocked by walls
        this.jumping_ = true;
        ...
        (this.right_leg_.getPart(1) as BodyGroup).getPart(1).setRotation(20 - 40 * this.animationProgress_);
        ...
        (this.left_leg_.getPart(1) as BodyGroup).getPart(1).setRotation(20 - 40 * this.animationProgress_);
        ...
        let obstacle: Tile = this.getCharObstacle(this.direction_);
        let offset: number = x * direction;
        if (direction == 1) {
    ...
    private chargeJump() {
        ...
        (this.right_leg_.getPart(1) as BodyGroup).getPart(1).setRotation(0 + 20 * this.animationProgress_);
    ...
    private land() {
        ...
        this.left_leg_.setRotation(30 - 30 * this.animationProgress_);
        this.left_leg_.getPart(1).setRotation(-50 + 50 * this.animationProgress_);
        (this.left_leg_.getPart(1) as BodyGroup).getPart(1).setRotation(20 - 20 * this.animationProgress_);
    ...
    private chargeRevert(animInit: number, yPos: number) {
        ...
        (this.right_leg_.getPart(1) as BodyGroup).getPart(1).setRotation(animInit * 20 - animInit * 20 * this.animationProgress_);
        ...
        (this.left_leg_.getPart(1) as BodyGroup).getPart(1).setRotation(20 * animInit - animInit * 20 * this.animationProgress_);
    ...
    private roll() { // needs to be affected by gravity & blocked by walls
        this.stop();
        this.followingOff();
        this.lock_ = true;
        this.rolling_ = true;
    ...
    private knifeHit() {
        if (this.holstering_) {
            if (this.animationProgress_ < 0) {
                ...
                this.toggleKnife();
                (this.left_arm_.getPart(1) as BodyGroup).getPart(1).masquer();
                (this.left_arm_.getPart(1) as BodyGroup).getPart(0).changeImage("assets/bottom_arm_part.svg");
                this.animate(() => this.idle(), 50);
            } else if (this.animationProgress_ >= 1) {
                this.animationDirection_ *= -1;
            }
            ...
            if (this.hasKnife_ && this.animationProgress_ >= 0.05) {
                this.toggleKnife();
                (this.left_arm_.getPart(1) as BodyGroup).getPart(0).changeImage("assets/bottom_arm_part_holding_knife.svg");
                (this.left_arm_.getPart(1) as BodyGroup).getPart(1).montrer();
            }
    ...
    private recoil() {
        ...
        let AnimationDirection: number = 1;

        // initial position
        let LeftArmAngle: number = this.left_arm_.getRotation();
        let RightArmAngle: number = this.right_arm_.getRotation();
        ...
        let int: number = window.setInterval(anim, 15);
    }
    public updateRifleStats(atkSpeed: number, range: number, mag: number, reloadSpeed: number) {
        /* stats update for in game upgrades */
        (this.rifle_ as Gun).setStats(atkSpeed, range, mag, reloadSpeed);
    }
    public getRifleStats() {
        return (this.rifle_ as Gun).getStats();
    }
    public followingOn() {
        this.mouse_ = (e: MouseEvent) => { this.follow(e); };
    ...
    private follow(e: MouseEvent) {
        if (!this.holstering_) {
            ...
            let sx: number = e.clientX - this.scene_.getX();
            let sy: number = e.clientY - this.scene_.getY() + 1.5 * this.rifle_.getHauteur();
            /* where is reference center */
            let px: number = 0.55 * this.left_arm_.getPart(0).getLargeur() + this.left_arm_.getX() + this.getX();
            let py: number = 0.21 * this.left_arm_.getPart(0).getHauteur() + this.left_arm_.getY() + this.getY();
            ...
            let angle: number = Math.acos(dx / d) / Math.PI * 180 - 180;
        ...
        /* turn with mouse */
        let treshold: number = e.clientX - this.scene_.getX() - this.getX() - 0.47 * this.head_.getPart(0).getLargeur();
    ...
    private moveRifle(progress: number) {
        ...
        (this.rifle_ as Gun).inflexion_ = progress;
    }
    /* Keyboard events */
    public KeyDown: any = (e: KeyboardEvent) => {
        if (!this.frozen_) {
            if (e.key == this.scene_.KeyBindings_.Left && this.jumping_ == false && this.lock_ == false) {
                let obstacle: Tile = this.getCharObstacle("left");
                this.turn("left");
                ...
                let obstacle: Tile = this.getCharObstacle("right");
                this.turn("right");
                if (obstacle != undefined) {
    ...
    public KeyUp: any = (e: KeyboardEvent) => {
        if (!this.frozen_) {
            if (e.key == this.scene_.KeyBindings_.Down && this.charged_ == true && this.jumping_ == false) {
                ...
                /* init revert */
                let init: number = this.animationProgress_.valueOf();
                let yPos: number = this.originalYPos_.valueOf();
                this.initPos_ = [
                    this.left_arm_.getRotation(),
                    this.left_arm_.getPart(1).getRotation(),
                    this.right_arm_.getRotation(),
     ...
     */
    private charFall(target: Tile) {
        let increment: number = 100 * this.scale_; //~= falling speed
        let yOffSet: number = 40 * this.scale_; //= idle offset

        /* seting up fall parameters */
        let fallConfigured: Function = () => {
            this.fall(
                target,
                increment,
                yOffSet,
    ...
    private getCharObstacle(direction: string): Tile {
        let yOffSet: number = 40 * this.scale_; //= idle offset
